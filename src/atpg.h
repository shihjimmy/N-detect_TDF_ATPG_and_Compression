...
class ATPG {
 public:
  ...
  void set_DTC_only(const bool &);
  void set_STC_only(const bool &);
  ...
  int detected_num{};
  bool get_tdf_only() { return tdfsim_only | tdfatpg_only; }
 ...
 private:
  ...
  /* declared in tdf_podem.cpp */
  vector<int> last_PIs, last_PIs_DTC;
  int v2_shiftin;
  int v1_shift_out;
  vector<vector<int>> Ndet_TPG_V1_test_pattern_set;
  vector<int> Ndet_TPG_V2_set;

  int tdf_podem(fptr, int &);
  bool check_vector1(fptr);

  /* declared in compress.cpp*/
  // For dynamic test compression
  int  tdf_podem_with_compression(fptr, int &);
  void tdf_podem_x();
  int  tdf_podemx_second(const fptr&);
  void STC_compression(int &, int &);
  ...
  class WIRE {
   public:
    ...
    forward_list<fptr> wire_flist; /* for dynamic test compression */
