...
void ATPG::transition_delay_fault_simulation(int &total_detect_num) {
  ...
  for (i = vectors.size() - 1; i >= 0; i--) {
    //std::cout<<distance(flist_undetect.begin(), flist_undetect.end())<<"faults\n";
...
/*
  make some changes in tdf simulation
  in order to do the static test compression
*/
...
bool ATPG::tdfault_sim_a_vector2(const string &vec, int &num_of_current_detect) {  
  ...
  /* for every input, set its value to the V2 value */
      ...
      [&](const fptr fptr_ele) {
        if (fptr_ele->detect == TRUE) {
            ...
            if (fptr_ele->detected_time >= detected_num){
                ...
                sort_wlist[fptr_ele->to_swlist]->wire_flist.remove(fptr_ele);
                return true;
            } 
            else {
...
void ATPG::generate_tdfault_list() {
  ...
  for (auto pos = sort_wlist.crbegin(); pos != sort_wlist.crend(); ++pos) {
    ...
    sort_wlist[flist_undetect.front()->to_swlist]->wire_flist.push_front(flist_undetect.front());
