...
/* parse the input switches & arguments */
  while (i < argc) {
    ...
    // for compression
      ...
      atpg.set_DTC_only(true);
      i++;
    } 

    else if (strcmp(argv[i], "-STC") == 0) {
      atpg.set_STC_only(true);
      i++;
    } 
...
/* if vector file is provided, read it */
  ...
  if (!atpg.get_tdf_only()) atpg.generate_fault_list(); //init_flist.cpp
  else atpg.generate_tdfault_list();
  atpg.timer(stdout, "for generating fault list");

  if (atpg.get_tdfatpg() && atpg.get_compression()){
    // test with static compression and dynamic compression
    // but only in tdf mode
    atpg.test_with_compression();  //compress.cpp
  }
  else{
    // original test
    atpg.test();  //atpg.cpp
  ...
  if (!atpg.get_tdf_only()) atpg.compute_fault_coverage(); //init_flist.cpp
...
bool ATPG::get_tdfatpg() {
  return this->tdfatpg_only;
}
...
void ATPG::set_STC_only(const bool &b) {
  this->compression = b;
  this->STC_only = b;
}

void ATPG::set_DTC_only(const bool &b) {
  this->compression = b;
  this->DTC_only = b;
}

bool ATPG::get_compression() {
